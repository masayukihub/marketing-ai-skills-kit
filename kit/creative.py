...
from .review import unit_items, email_items
...
def page_report(root: Path, project: dict, scope: str, as_of: date) -> tuple[dict, str]:
    ...
    rows = []; sections = {}; problems = []; review_items = []
    for unit in project.get('page_units', []):
        ...
        scoped_reviews = unit_items(root, project, unit, as_of)
        review_items.extend(scoped_reviews)
        copy_states = {r['field']: r['status'] for r in scoped_reviews if r['category'] == 'copy'}
        for item in scoped_reviews:
            if item['status'] != 'APPROVED_INTERNAL_ONLY':
                problems.append({'unit': unit['id'], 'code': 'SCOPED_REVIEW_REQUIRED', 'item_id': item['item_id']})
        ...
        row['copy_review'] = copy_states
        rows.append(row)
        bound_text = '<br>'.join(esc(c['text']) + (f' — {esc(c["conditions"])}' if c.get('conditions') else '') for c in bound if claim_usable(c, project, as_of))
        warning = '<p class="warn">コピー候補・要確認：' + esc(' / '.join(copy_states.values())) + '。事実・外部公開の承認ではありません。</p>'
        if blocked:
            warning += '<p class="warn">参照された主張に未承認または期限切れのものがあります。確定コピーとして使用しないでください。</p>'
    ...
    return {'scope': scope, 'units': rows, 'issues': problems, 'review_items': review_items, 'publication_ready': False,
...
def edm_html(project: dict, as_of: date, root: Path | None = None) -> tuple[dict, str]:
    edm = project.get('edm', {})
    url = edm.get('cta_url', '')
    parsed = urlparse(url)
    valid_url = bool(parsed.scheme in {'https', 'http'} and parsed.netloc and not parsed.username and not parsed.password)
    selected_ids = edm.get('claim_ids', [c['id'] for c in project.get('claims', [])])
    selected = [c for c in project.get('claims', []) if c['id'] in selected_ids]
    approved = [c for c in selected if claim_usable(c, project, as_of)]
    review_items = email_items(root, project, as_of)
    copy = ''.join('<p style="margin:12px 0">'+esc(c['text'])+((' '+esc(c['conditions'])) if c.get('conditions') else '')+'</p>' for c in approved)
    copy = '<p style="padding:12px;background:#fff5df">COPY CANDIDATE / 未確認の表現は要レビュー。外部公開・送信は未承認。</p><p>'+esc(edm.get('body'))+'</p>' + copy
    ...
    return {'subject': edm.get('subject'), 'preheader': edm.get('preheader'),
            ...
            'blocked_claim_ids': [c['id'] for c in selected if not claim_usable(c, project, as_of)]}, content
